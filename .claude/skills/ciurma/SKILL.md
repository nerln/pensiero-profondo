---
name: ciurma
description: Run and drive a ciurma crew from this Claude Code session. Use when the user wants to start the hub, open the crew window, sign members on, read or write the board, attack a claim, or check what the crew is doing.
---

# ciurma from inside Claude Code
...
2. If no `.ciurma/` exists in the project: `node dist/cli.js init .`
3. Start the hub and open it in the browser pane with the `preview_start` tool, configuration name `ciurma` (from `.claude/launch.json`). The UI is at http://127.0.0.1:4177 and needs no token on this machine.

## Drive the crew without leaving the terminal

If the `ciurma` MCP server is connected to this session (`claude mcp add ciurma -- node dist/cli.js mcp .`), use its tools:
...
Without the MCP server, the REST API works with the token from `.ciurma/config.json` in the `x-ciurma-token` header (see README).
