...
try {
  ...
  const ran = await until('the Bash result in the transcript', async () => { const t = await api('GET', `/api/members/${member.id}/transcript`); return t.find((i) => i.kind === 'tool_result' && /^42\s*$/m.test(i.text)) ?? null; }, 90000);
