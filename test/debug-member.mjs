...
const dir = mkdtempSync(join(tmpdir(), 'pensiero-dbg-'));
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
let TOKEN = '';
const api = async (m, p, b) => { const r = await fetch(`http://127.0.0.1:${PORT}${p}`, { method: m, headers: { 'content-type': 'application/json', 'x-pensiero-token': TOKEN }, body: b ? JSON.stringify(b) : undefined }); return r.json(); };
await new Promise((r) => spawn('node', [CLI, 'init', dir, '--port', String(PORT)]).on('exit', r));
TOKEN = JSON.parse((await import('node:fs')).readFileSync(join(dir, '.pensiero', 'config.json'), 'utf8')).token;
...
const member = await api('POST', '/api/members', { roleId: 'deckhand', machineId: machine.id, name: 'Dbg', model: process.env.PENSIERO_MODEL || 'claude-sonnet-5', effort: 'low', cwd: dir, brief: 'Write exactly one entry on the board with lavagna_scrivi: verb "fatto", text "e2e ready", to "all". Then reply with the single word done and stop.' });
